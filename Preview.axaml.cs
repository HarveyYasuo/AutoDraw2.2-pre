...
public partial class Preview : Window
{
    ...
    private bool drawingColorLayers;
    private List<InputAction> actions = new();
    private List<SKBitmap> stack = new();
    private List<ColorLayerData> colorLayers = new();
    ...
    private void Keybind(object? sender, KeyboardHookEventArgs e)
    {
        ...
        {
            if (inputBitmap is null && !drawingStack && !drawingColorLayers) return;
            Thread drawThread = new(async () =>
            {
                ...
                try
                {
                    if (drawingStack)
                    {
                        success = await Drawing.DrawStack(stack, actions, Drawing.LastPos);
                    }
                    else if (drawingColorLayers)
                    {
                        success = await Drawing.DrawColorLayers(colorLayers, actions, Drawing.LastPos);
                    }
                    else
                    {
                        success = await Drawing.Draw(inputBitmap, Drawing.LastPos);
                    }
                }
                catch { }
                ...
                Dispatcher.UIThread.Invoke(() =>
                {
                    if (Application.Current.ApplicationLifetime is IClassicDesktopStyleApplicationLifetime desktop)
                    {
                        ...
                        {
                            if (success)
                                mw.ImageInfoText.Content = "Dibujo completado!";
                            else
                                mw.ImageInfoText.Content = "Dibujo cancelado.";
                        }
    ...
    public void ReadyColorLayerDraw(Bitmap preview, List<ColorLayerData> layers, List<InputAction> _actions)
    {
        drawingStack = false;
        drawingColorLayers = true;
        inputBitmap = null;

        renderedBitmap?.Dispose();
        renderedBitmap = preview;
        PreviewImage.Source = renderedBitmap;
